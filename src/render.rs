...
pub fn render(ppu: &PPU, frame: &mut Frame) {
    ...
    let (main_nametable, second_nametable) = match (&ppu.mirroring, ppu.ctrl.nametable_addr()) {
        (cartridge::Mirroring::Vertical, 0x2000)
        | (cartridge::Mirroring::Vertical, 0x2800)
        | (cartridge::Mirroring::Horizontal, 0x2000)
        | (cartridge::Mirroring::Horizontal, 0x2400) => {
            (&ppu.vram[0..0x400], &ppu.vram[0x400..0x800])
        }
        (cartridge::Mirroring::Vertical, 0x2400)
        | (cartridge::Mirroring::Vertical, 0x2C00)
        | (cartridge::Mirroring::Horizontal, 0x2800)
        | (cartridge::Mirroring::Horizontal, 0x2C00) => {
